...
/***/ function(module, exports, __webpack_require__) {
	...
	var addListeners = __webpack_require__(1);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var archives = __webpack_require__(2);
	var dropbox = __webpack_require__(4);
	var DropboxAuthenticator = __webpack_require__(5);

	var StorageInterface = window.Buttercup.Web.StorageInterface;

	var RESPOND_ASYNC = true;
	var RESPOND_SYNC = false;

	function getEntriesForURL(url) {
	    var matchingEntries = archives.getMatchingEntriesForURL(url).map(function (entry) {
	        return {
	            id: entry.getID(),
	...
	module.exports = function addListeners() {
	    ...
	    chrome.runtime.onMessage.addListener(function (request, sender, sendResponse) {
	        switch (request.command) {
	            ...
	            case "add-archive":
	                {
	                    var archiveData = request.data;
	                    archives.addArchiveByRequest(archiveData).then(function (result) {
	                        sendResponse({
	                            ok: true
	                        });
	                    }).catch(function (err) {
	            ...
	            case "authenticate-dropbox":
	                {
	                    var _ret = function () {
	                        var dba = new DropboxAuthenticator();
	                        dba.authenticate().then(function () {
	                            sendResponse({
	                                ok: true,
	                                token: dba.token
	                            });
	            ...
	            case "get-archive-states":
	                {
	                    var states = archives.getArchiveList();
	                    sendResponse(states);
	                    break;
	                }
	            ...
	            case "get-entries-for-url":
	                {
	                    var matchingEntries = getEntriesForURL(request.url);
	                    sendResponse({
	                        ok: true,
	            ...
	            case "lock-archive":
	                {
	                    archives.lockArchive(request.name).then(function () {
	                        sendResponse({
	                            ok: true
	                        });
	                    }).catch(function (err) {
	            ...
	            case "remove-archive":
	                {
	                    var removed = archives.removeArchive(request.name);
	                    if (removed) {
	                        sendResponse({
	                            ok: true
	                        });
	            ...
	            case "save-form-submission":
	                {
	                    var _matchingEntries = getEntriesForURL(request.data.url);
	                    if (_matchingEntries <= 0) {
	            ...
	            case "save-new-entry":
	                {
	                    var data = request.data,
	                        ...
	                        _entry = void 0;
	                    try {
	                        var result = archives.createEntry(data.archiveID, data.groupID, data.title);
	                    ...
	                    _entry.setProperty("username", data.username).setProperty("password", data.password).setMeta("URL", data.url).setMeta("LoginURL", data.loginURL);
	            ...
	            case "unlock-archive":
	                {
	                    ...
	                    archives.unlockArchive(opts.name, opts.password).then(function () {
	                        sendResponse({
	                            ok: true
	                        });
	                    }).catch(function (err) {
...
/***/ function(module, exports, __webpack_require__) {
	...
	var Archive = Buttercup.Archive,
	    ...
	    WebDAVDatasource = Buttercup.WebDAVDatasource,
	    OwnCloudDatasource = Buttercup.OwnCloudDatasource,
	    SharedWorkspace = Buttercup.SharedWorkspace;

	var DropboxDatasource = Buttercup.Web.DropboxDatasource;

	function groupToSkeleton(item, isGroup) {
	    var subItems = item.getGroups().map(function (group) {
	        return groupToSkeleton(group, true);
	    });
	    if (!isGroup) {
	        // archive
	        return {
	...
	function validateAndSave(name, workspace, credentials, password) {
	    return Promise.resolve(workspace).then(validation.validateWorkspace).then(function () {
	        Buttercup.Web.archiveManager.addArchive(name, workspace, credentials, password);
	        return Buttercup.Web.archiveManager.saveState();
	    });
	}

	var archives = module.exports = {

	    addArchiveByRequest: function addArchiveByRequest(request) {
	        return Promise.resolve(request).then(validation.validateArchiveAddition).then(function () {
	            switch (request.type) {
	                case "webdav":
	                    {
	    ...
	    addDropboxArchive: function addDropboxArchive(request) {
	        return Promise.resolve(request)
	        // .then(validation.validateDropboxArchive)
	        .then(function () {
	            var dropboxCreds = new Credentials();
	            ...
	            var datasource = new DropboxDatasource(request.dropbox_token, request.dropbox_path);
	            if (request.connect === "new") {
	                var _ret = function () {
	                    var workspace = new SharedWorkspace();
	                    workspace.setPrimaryArchive(Archive.createWithDefaults(), datasource, request.master_password);
	                    return {
	                        v: workspace.save().then(function () {
	            ...
	            return archives.fetchWorkspace(datasource, request.master_password).then(function (workspace) {
	                return [workspace, credentials];
	            });
	        }).then(function () {
	            ...
	            return validateAndSave(request.name, workspace, credentials, request.master_password);
	        });
	    },

	    addOwnCloudArchive: function addOwnCloudArchive(request) {
	        return Promise.resolve(request).then(function () {
	            var owncloudCreds = new Credentials();
	            ...
	            return archives.fetchWorkspace(new OwnCloudDatasource(request.owncloud_address, request.owncloud_path, request.owncloud_username, request.owncloud_password), request.master_password).then(function (workspace) {
	                return [workspace, credentials];
	            });
	        }).then(function () {
	    ...
	    addWebDAVArchive: function addWebDAVArchive(request) {
	        return Promise.resolve(request).then(function () {
	            var webdavCreds = new Credentials();
	            ...
	            return archives.fetchWorkspace(new WebDAVDatasource(request.webdav_address, request.webdav_path, request.webdav_username, request.webdav_password), request.master_password).then(function (workspace) {
	                return [workspace, credentials];
	            });
	        }).then(function () {
	    ...
	    createEntry: function createEntry(archiveID, groupID, title) {
	        var unlockedArchives = Buttercup.Web.archiveManager.unlockedArchives,
	            numArchives = unlockedArchives.length,
	            archive = void 0,
	            workspace = void 0;
	        for (var i = 0; i < numArchives; i += 1) {
	            var thisArchive = unlockedArchives[i].workspace.primary.archive;
	    ...
	    fetchWorkspace: function fetchWorkspace(datasource, masterPassword) {
	        return datasource.load(masterPassword).then(function (archive) {
	            var workspace = new SharedWorkspace();
	    ...
	    getArchiveList: function getArchiveList() {
	        return Buttercup.Web.archiveManager.displayList;
	    },

	    getEntry: function getEntry(archiveID, entryID) {
	        var unlockedArchives = Buttercup.Web.archiveManager.unlockedArchives,
	            numArchives = unlockedArchives.length;
	        // try archive ID first:
	        for (var i = 0; i < numArchives; i += 1) {
	            var archive = unlockedArchives[i].workspace.primary.archive,
	        ...
	        for (var _i = 0; _i < numArchives; _i += 1) {
	            var _archive = unlockedArchives[_i].workspace.primary.archive,
	                entry = _archive.getEntryByID(entryID);
	            if (entry) {
	                return entry;
	            }
	        }
	    ...
	    getMatchingEntriesForURL: function getMatchingEntriesForURL(url) {
	        var unlockedArchives = Buttercup.Web.archiveManager.unlockedArchives,
	            entries = [];
	        unlockedArchives.forEach(function (archiveItem) {
	            var archive = archiveItem.workspace.primary.archive;
	            var newEntries = Buttercup.Web.ArchiveTools.getEntriesForURL(archive, url);
	    ...
	    lockArchive: function lockArchive(name) {
	        return Buttercup.Web.archiveManager.lock(name);
	    },

	    mapArchivesToSkeleton: function mapArchivesToSkeleton() {
	        var unlockedArchives = Buttercup.Web.archiveManager.unlockedArchives;
	        return unlockedArchives.map(function (item) {
	            return Object.assign(groupToSkeleton(item.workspace.primary.archive), {
	                name: item.name
	            });
	        });
	    },

	    removeArchive: function removeArchive(name) {
	        return Buttercup.Web.archiveManager.removeArchive(name);
	    },

	    unlockArchive: function unlockArchive(name, password) {
...
/***/ function(module, exports) {
	...
	var validate = module.exports = {
	    ...
	    archiveNameAvailable: function archiveNameAvailable(name) {
	        return Buttercup.Web.archiveManager.archives.hasOwnProperty(name) === false;
	    },

	    validateArchiveAddition: function validateArchiveAddition(request) {
	    ...
	    validateObjectString: function validateObjectString(obj, property) {
	        if (!obj[property]) {
	    ...
	    validateWorkspace: function validateWorkspace(workspace) {
	        if (!workspace) {
	            throw new Error("Workspace is undefined");
	        }
	        var _workspace$primary = workspace.primary,
	            archive = _workspace$primary.archive,
	            datasource = _workspace$primary.datasource,
	            password = _workspace$primary.password;
...
/***/ function(module, exports, __webpack_require__) {
	...
	var dbTools = __webpack_require__(4);

	var DropboxAuthenticator = function () {
	    function DropboxAuthenticator(accessToken) {
	        _classCallCheck(this, DropboxAuthenticator);

	        this._token = accessToken || null;
	        this._client = accessToken ? new Dropbox({ accessToken: accessToken }) : new Dropbox({
	            clientId: "5fstmwjaisrt06t"
	        });
	    }

	    _createClass(DropboxAuthenticator, [{
	        key: "authenticate",
...
/***/ function(module, exports, __webpack_require__) {
	...
	var DropboxBase = __webpack_require__(8);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var REQUEST_CONSTANTS = __webpack_require__(9);
	var DropboxBase;
	...
	DropboxBase.prototype.getRpcRequest = function () {
	  if (DropboxBase.prototype.rpcRequest === undefined) {
	    DropboxBase.prototype.rpcRequest = __webpack_require__(11);
	  }
	...
	DropboxBase.prototype.getDownloadRequest = function () {
	  if (DropboxBase.prototype.downloadRequest === undefined) {
	    DropboxBase.prototype.downloadRequest = __webpack_require__(22);
	  }
	...
	DropboxBase.prototype.getUploadRequest = function () {
	  if (DropboxBase.prototype.uploadRequest === undefined) {
	    DropboxBase.prototype.uploadRequest = __webpack_require__(23);
	  }
...
/***/ function(module, exports, __webpack_require__) {
	...
	var getBaseURL = __webpack_require__(21);
